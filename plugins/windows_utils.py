...
class WindowsUtils:
    ...
    def rename_file(self, file_path, new_name):
        ...
        new_path = new_name
        if os.path.dirname(file_path) and not os.path.dirname(new_name):
            new_path = os.path.join(os.path.dirname(file_path), new_name)

        if os.path.exists(new_path):
            print(f"A file already exists with that name: {new_path}")
            return False
        ...
        try:
            ...
            print(f"Failed to rename {file_path}: {error}")
            return False
